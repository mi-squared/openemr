...
echo  "PID: " . $pid ." ... ENCOUNTER: ". $encounter;
...
if(filter_input(INPUT_GET, 'com') == 1){
    ...
    interactive_c($encounter, $pid);
    	...
    	function eSign_checkout($pid, $encounter, $m, $t=''){
             ...
             function timeUpdate($pid, $en){
              ...
              function cdiag($pid){
                    ...
                    //retrieve the last diagnosis for the patient
                    $c_sql = "SELECT form_id FROM forms WHERE form_name ='Patient Diagnosis' AND pid=$pid ORDER BY encounter DESC";
                    
					$f_id = sqlStatement($c_sql);

                    while ($res = sqlFetchArray($f_id)) {
	
		                //echo "cdiag res " . print_r($res, 1) . "<br><br>" . $c_sql;
						
						$diag = sqlStatement("SELECT field_value FROM lbf_data WHERE form_id = '" . $res['form_id'] . "'");
						$row = sqlFetchArray($diag);
						
						if ($row['field_value']) {
							//echo "<br>DIAG: " . $row['field_value'];
			  ...
			  function bInfo($pid, $encounter){
				  ...
				  // check to see if the user is LCPC, LPC, LCSW, for billing purpose
				  /*
				  $sql = "SELECT info FROM users WHERE id = '$prov'";
				  $checkProv = sqlStatement($sql);
				  $provStatus = sqlFetchArray($checkProv);
				  $provInfo = $provStatus['info'];
				  
				    if($provInfo == "" || $provInfo == NULL){
						   $prov = 15;
					   }
				  */
		...
		if(!empty($js[0])){
                ...
                if ($res['encounter'] != $encounter){ 
				  ...
				  echo "<script>window.close();</script>";
	              	...
	              	echo "<br/><b><font color='red'>No Diagnosis has been entered, <br/><br/>Patient cannot be checked out. </font></b>"; exit;
