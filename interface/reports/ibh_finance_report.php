...
function getBillingItems($encounter, $ins_id) {
	
	  $feesq = sqlStatement("SELECT code, fee FROM billing WHERE encounter='$encounter' AND fee > 0");
	  $fees_html = "";
	  $ins_est = "";
	  $code = "";
	  $ct = 0;
	  ...
	  while ($fee = sqlFetchArray($feesq)){
		  $ct++;
		  $code = $fee['code'];
		   
		  $amtq = sqlStatement("SELECT amount FROM ibh_insurance_est WHERE ins_id='$ins_id' AND encounter_code='$code' LIMIT 1");
		  $allowed = sqlFetchArray($amtq)['amount'];
		  
		  $exp += $allowed;
		  
		  $fees_html .= "<div>" . $code . "&nbsp; fee: " . $fee['fee'] . " allowed: $" . $allowed . "</div>";
	   }
	  
	  return array("html"=>$fees_html, "allowed"=>$exp);
}
...
function endPatient($ptrow) {
  ...
  if ($_POST['form_export']) {
    ...
    if ($ptrow['count'] > 1) {
     ...
     /*
	     // SKIPPING PATIENT TOTAL ROW HERE
...
<title>IBH FINANCE REPORT</title>
<style type="text/css">
...
<span class='title'>IBH FINANCE REPORT</span>

<form method='post' action='ibh_finance_report.php' enctype='multipart/form-data' id='theform'>
...
if ($_POST['form_refresh'] || $_POST['form_export'] || $_POST['form_csvexport']) {
  ...
  $sqlArray = array(); 
    ...
    while ($erow = sqlFetchArray($eres)) {
      ...
      // IBH_DEV
      // grabbing individual billing items that have fees
      ...
      if ($insposition == 1) {
        ...
        if (!empty($payerids)) {
	      $row['ins1_id'] = $payerids[0];
          $row['ins1'] = getInsName($payerids[0]);
        }
      }
      
      // echo $encounter_id . "<br>";
...
<?php } ?>
  ...
  <th align="right" class='allowed'>Allowed&nbsp;</th>
...
<?php
  ...
  foreach ($rows as $key => $row) {
	...
	// echo "<tr><td colspan='10'>" . print_r($row, true) . "</td></tr>";
...
<?php
	  ...
	  if ($ptrow['count'] != 99) {
		...
		$fees_html = getBillingItems($row['encounter'], $row['ins1_id']);
        ...
        if ($form_cb_ins1 || $form_payer_id ) {
          echo "  <td class='detail'>&nbsp;" . attr($row['ins1']) . ":" . $row['ins1_id'] . "</td>\n";
        }
        if ($form_provider) {
...
<?php } ?>
  ...
  <?php
	  ...
	  $grand_total_allowed += $fees_html['allowed'];
  ?>
  <td class="detail allowed" align="right"><?=number_format($fees_html['allowed'],2)?></td>
...
<tr><td colspan=10><?=$fees_html['html']?></td></tr>
...
<?php
  ...
  if ($_POST['form_export']) {
    ...
    echo "<td class='dehead' align='right'>" . oeFormatMoney($grand_total_allowed) . "</td>";
