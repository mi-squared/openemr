...
//Added by Sherwin
//--------------------------------------------------------------------------------------------------------------//
function fetchPayrollData($form_apptstatus, $from_date, $to_date, $provider )
{

  if($provider){
	$sql = "SELECT b.id, b.code, b.provider_id, b.code_text, b.units, count('code_text'), u.id, u.fname AS ufname, u.lname AS ulname FROM `billing` AS b 
	        LEFT OUTER JOIN users AS u ON u.id = b.provider_id
	         WHERE `provider_id` = ? AND date >= ? AND date <= ?  
			 GROUP BY code_text ASC";
	
	$appointments = array();
	$payroll = sqlStatement($sql, array($provider, $from_date, $to_date));
    
	while ( $row = sqlFetchArray($payroll)){
		$appointments[] = $row;
	}
    ...
    return $appointments;	
	  ...
	  $to = $to_date . "00:00:00";
	  $providers = array();
      $sql = "SELECT provider_id FROM billing WHERE date >= '$from' AND date <= '$to' group by provider_id";
      $providerList = sqlStatement($sql);
	  
	  while($row = sqlFetchArray($providerList)){
		  $providers[] = $row;
	  }
...
function fetchUnits($providerId, $code_text, $from_date, $to_date){
	                    $sql = "SELECT units FROM `billing` WHERE `provider_id` = ? AND `code_text` LIKE '%$code_text%' AND date >= ? AND date <= ?"; 
						$res = sqlStatement($sql, array($providerId, $from_date, $to_date));

						$i = 0;
						while ($units = sqlFetchArray($res)){

							$i = $i + $units['units']; //total up the units
